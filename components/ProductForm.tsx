  ...
  return (
    <form action={saveProductAction} className="grid gap-6 lg:grid-cols-[1fr_360px]">
      ...
      <section className="space-y-5 rounded-lg border border-line bg-white p-5">
        <Field label="商品名称" name="name" defaultValue={product?.name} required />
        <Field label="Slug" name="slug" defaultValue={product?.slug} placeholder="留空会按商品名生成" />
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="品牌" name="brand" defaultValue={product?.brand} required />
          <Field label="日本参考价格（日元）" name="priceJpy" type="number" defaultValue={product?.priceJpy} required />
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="到货周期" name="deliveryCycle" defaultValue={product?.deliveryCycle || "7-14 天"} required />
        </div>
        <Field label="短介绍" name="shortIntro" defaultValue={product?.shortIntro} required />
        <TextArea label="商品介绍" name="description" defaultValue={product?.description} required />
        <TextArea label="商品规格" name="specs" defaultValue={product?.specs} required />
        <TextArea label="注意事项" name="notes" defaultValue={product?.notes || ""} />
      </section>
      <aside className="space-y-5">
        <section className="space-y-4 rounded-lg border border-line bg-white p-5">
          ...
          <label className="text-sm font-medium text-ink">
            ...
            <select name="stockStatus" defaultValue={product?.stockStatus || "AVAILABLE"} className="mt-2 h-10 w-full rounded-md border border-line px-3 outline-none focus:border-ink">
              <option value="AVAILABLE">可代购</option>
              <option value="LIMITED">少量可代购</option>
              <option value="PREORDER">可预订</option>
              <option value="UNAVAILABLE">暂不可代购</option>
            </select>
          </label>
        ...
        <section className="rounded-lg border border-line bg-white p-5">
          <h2 className="mb-3 text-sm font-semibold text-ink">商品图片</h2>
          <ImageUploader defaultValue={product?.images.map((image) => image.url).join("\n") || ""} />
        </section>
        <button className="w-full rounded-md bg-ink px-4 py-3 text-sm font-semibold text-white hover:bg-gold">保存商品</button>
      </aside>
    </form>
  );
}
