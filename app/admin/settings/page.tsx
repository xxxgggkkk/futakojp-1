...
export default async function SettingsPage() {
  ...
  return (
    <AdminShell>
      <h1 className="text-2xl font-semibold text-ink">网站设置</h1>
      <form action={saveSettingsAction} className="mt-6 grid gap-6 lg:grid-cols-[1fr_360px]">
        <section className="space-y-5 rounded-lg border border-line bg-white p-5">
          <Field label="网站名称" name="siteName" defaultValue={setting.siteName} />
          <Field label="SEO 标题" name="metaTitle" defaultValue={setting.metaTitle} />
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="微信号" name="wechatId" defaultValue={setting.wechatId || ""} />
            ...
            <Field label="邮箱" name="email" defaultValue={setting.email || ""} />
          </div>
          <button className="rounded-md bg-ink px-4 py-3 text-sm font-semibold text-white hover:bg-gold">保存设置</button>
        </section>
        <aside className="space-y-5">
          <section className="rounded-lg border border-line bg-white p-5">
            <h2 className="mb-3 text-sm font-semibold text-ink">Logo 图片</h2>
            <ImageUploader name="logoUrl" defaultValue={setting.logoUrl || ""} />
          </section>
          <section className="rounded-lg border border-line bg-white p-5">
            <h2 className="mb-3 text-sm font-semibold text-ink">微信二维码</h2>
            <ImageUploader name="wechatQrUrl" defaultValue={setting.wechatQrUrl || ""} />
          </section>
          <section className="rounded-lg border border-line bg-white p-5">
            <h2 className="mb-3 text-sm font-semibold text-ink">LINE 二维码</h2>
